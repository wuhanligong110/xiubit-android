...
public class TextDecorator {
    ...
    private TextDecorator(TextView textView, String content) {
        ...
        textView.setHighlightColor(ContextCompat.getColor(textView.getContext(), android.R.color.transparent));  //解决点击后背景色变色问题
    }
    ...
    public TextDecorator setAbsoluteSize(final int size, final String... texts) {
        ...
        for (String text : texts) {
            if (content.contains(text)) {
                List<Integer> indexs = searchAllIndex(text);
                for (int index : indexs) {
//                    Logger.d("decoratedContent:"+index);
    ...
    private ArrayList<Integer> searchAllIndex(String targetStr) {
        ArrayList indexs = new ArrayList<Integer>();
        int i = content.indexOf(targetStr);//*第一个出现的索引位置
        if(i!= -1) indexs.add(i);
        while (i != -1) {
            i = content.indexOf(targetStr, i + 1);//*从这个索引往后开始第一个出现的位置
            if(i != -1) indexs.add(i);
        }
        return indexs;
    }
