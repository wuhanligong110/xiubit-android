 ...
 * 可自动缩放的TextView 支持自定义字体
 */
public class AutoResizeTextView extends AppCompatTextView {
    private final String[] typeFacePaths = new String[]{"fonts/PingFang-Medium.ttf","fonts/DINOT-Medium.ttf"};
    ...
    public AutoResizeTextView(final Context context, final AttributeSet attrs, final int defStyle) {
        ...
        int typeFaceStyle = array.getInt(R.styleable.AutoResizeTextView_fontfit_typeface,0);

        AssetManager assets = getContext().getAssets();
        Typeface fromAsset = Typeface.createFromAsset(assets, typeFacePaths[typeFaceStyle]);
        setTypeface(fromAsset);

        // using the minimal recommended font size
        _minTextSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, 1, getResources().getDisplayMetrics());
