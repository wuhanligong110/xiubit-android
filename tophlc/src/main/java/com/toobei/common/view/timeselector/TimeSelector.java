package com.toobei.common.view.timeselector;
...
import com.toobei.common.view.timeselector.Utils.DateUtil;
import com.toobei.common.view.timeselector.Utils.ScreenUtil;
import com.toobei.common.view.timeselector.Utils.TextUtil;
import com.toobei.common.view.timeselector.view.PickerView;
...
public class TimeSelector {
    ...
    private Calendar startCalendar, endCalendar, curCalendar;
    ...
    public TimeSelector(Context context, ResultHandler resultHandler, String startDate, String endDate, String curDate) {
        this(context, resultHandler, startDate, endDate);
        curCalendar = Calendar.getInstance();
        curCalendar.setTime(DateUtil.parse(curDate, FORMAT_STR));
    }
    ...
    private void initDialog() {
        if (seletorDialog == null) {
            ...
            seletorDialog.setContentView(R.layout.layout_date_time_dialog_selector);
    ...
    private void loadComponent() {
        ...
        if (curCalendar == null) {
            ...
            year_pv.setSelected(getListIndex(year, curCalendar.get(Calendar.YEAR) + ""));
            month_pv.setSelected(getListIndex(month, (curCalendar.get(Calendar.MONTH) + 1) + ""));
            day_pv.setSelected(getListIndex(day, curCalendar.get(Calendar.DAY_OF_MONTH) + ""));
            hour_pv.setSelected(getListIndex(hour, curCalendar.get(Calendar.HOUR_OF_DAY) + ""));
            minute_pv.setSelected(getListIndex(minute, curCalendar.get(Calendar.MINUTE) + ""));
        }
        excuteScroll();
    }

    private int getListIndex(ArrayList<String> list, String curValue) {
        for (int i = 0; i < list.size(); i++) {
           // if (list.get(i).equals(curValue)) { // 这种判断会导致 "05" 和"5" 判断的结果是不等而返回0
                if (StringUtils.toInt(list.get(i))==StringUtils.toInt(curValue)) {
                return i;
            }
        }
        return 0;
