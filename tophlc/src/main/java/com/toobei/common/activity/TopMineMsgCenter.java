...
import com.toobei.common.data.MineMsgNoticeListAdapter;
import com.toobei.common.data.MineMsgPersonListAdapter;
 ...
 * @date 2015-10-26
 */
public abstract class TopMineMsgCenter extends TopBaseActivity implements OnRefreshListener2<ListView>, OnClickListener, CompoundButton.OnCheckedChangeListener, OnItemLongClickListener, OnItemClickListener {
    ...
    private void initTopTitle() {
        ...
        headerLayout.showTitle(TopApp.getInstance().getString(R.string.msg_center));
        headerLayout.showLeftBackButton();
        rightBtn = headerLayout.showRightTextButton(R.string.edit, new OnClickListener() {
            @Override
    ...
    private void initView() {
        ...
        bottomCheckBox.setTextColor(ContextCompat.getColor(ctx,setCursorColor()));
        bottomCheckBox.setOnCheckedChangeListener(this);
        TextView text_bottom_delete = (TextView) findViewById(R.id.text_bottom_delete);
        text_bottom_delete.setTextColor(ContextCompat.getColor(ctx,setCursorColor()));
    ...
    private void initViewPager() {
        ...
        msgTitleTV.setTextColor(ContextCompat.getColor(ctx,R.color.text_black_common));
    ...
    private View getNoticeListView() {
        ...
        noticeRefreshList.setMode(Mode.BOTH);
    ...
    private View getPersonListView() {
        if (personListLayout == null) {
            ...
            personListAdapter = new MineMsgPersonListAdapter(ctx, getDrawableId());
            ...
            personRefreshList.setMode(Mode.BOTH);
    ...
    private void initImageView() {
        ...
        cursor.setBackgroundResource(setCursorColor());
        ...
        public void onPageSelected(int arg0) {
            ...
            switch (arg0) {
                ...
                case 1:
                    ...
                    noticeTitleTV.setTextColor(ContextCompat.getColor(ctx,R.color.text_black_common));
                    ...
                    if (response.getCode().equals("0")) {
                        ...
                        if (type == 2) {
                            ...
                            if (response.getData().getPageCount() <= response.getData().getPageIndex() || response.getData().getPageCount() <= 1) {
                                noticeRefreshList.setMode(Mode.PULL_FROM_START);
                            }
                        } else if (type == 1) {
                            ...
                            pageIndexPerson++;
                            if (response.getData().getPageCount() <= response.getData().getPageIndex() || response.getData().getPageCount() <= 1) {
                                personRefreshList.setMode(Mode.PULL_FROM_START);
                            }
                        }
                    } else {
                        com.toobei.common.utils.ToastUtil.showCustomToast(response.getErrorsMsgStr());
    ...
    protected abstract Intent getIntentWebActivityCommon(TopBaseActivity activity);

    protected abstract int setCursorColor();

    @Override
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        //		Intent intent = new Intent(ctx, WebActivityCommon.class);
        Intent intent = getIntentWebActivityCommon(ctx);
        // TODO: 2016/11/15 0015 INFOMATION_URL_TYPE_NOTICE informationDetailUrl	机构动态 、资讯 、课堂详情链接
...
//        String  value = TopApp.getInstance().getDefaultSp().getInformationDetailUrl() //
