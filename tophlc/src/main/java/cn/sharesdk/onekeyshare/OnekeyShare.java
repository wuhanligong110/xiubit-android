...
public class OnekeyShare implements PlatformActionListener, Callback {
	...
	public String getText() {
		return shareParamsMap.containsKey("text") ? String.valueOf(shareParamsMap.get("text"))
				: null;
	}
	...
	private void showNotification(long cancelTime, String text) {
		try {
			...
			// TODO: 2017/2/18  android 6.0   Android Notification.setLatestEventInfo弃用和Notification.Builder
...
//			nm.notify(id, notification);
			...
			if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN) {
				notification = new  NotificationCompat.Builder(app)
						.setSmallIcon(notifyIcon)//必须要先setSmallIcon，否则会显示默认的通知，不显示自定义通知
						.setTicker(text)
				...
				notification = new Notification.Builder(app)
						.setSmallIcon(notifyIcon)//必须要先setSmallIcon，否则会显示默认的通知，不显示自定义通知
						.setTicker(text)
						.setContentTitle(text)
						.setContentText(text)
