...
public class EditPage extends EditPageFakeActivity implements OnClickListener, TextWatcher {
	...
	private RelativeLayout getPageView() {
		...
		if (dialogMode) {
			...
			RelativeLayout.LayoutParams lpDialog = new RelativeLayout.LayoutParams(width,
	...
	private void genBackground() {
		...
		if (backgroundView != null) {
			try {
				Bitmap bgBm = captureView(backgroundView, backgroundView.getWidth(),
	...
	public void afterPlatformListGot() {
		...
		for (int i = 0; i < size; i++) {
			...
			iv.setLayoutParams(new FrameLayout.LayoutParams(LayoutParams.MATCH_PARENT,
					LayoutParams.MATCH_PARENT));
			fl.addView(iv);
