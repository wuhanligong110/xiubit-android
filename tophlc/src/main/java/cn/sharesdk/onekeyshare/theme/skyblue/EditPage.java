...
public class EditPage extends EditPageFakeActivity implements OnClickListener, TextWatcher {
		...
		for (Platform platform : platforms) {
			...
			if (isShowAtUserLayout(platformName)) {
				View view = LayoutInflater.from(activity).inflate(
						getLayoutRes(activity, "skyblue_editpage_at_layout"), null);
				TextView atDescTextView = (TextView) view.findViewById(getIdRes(activity,
						"atDescTextView"));
				TextView atTextView = (TextView) view
						.findViewById(getIdRes(activity, "atTextView"));
				...
				atDescTextView.setText(getContext().getString(
						getStringRes(activity, "list_friends"), getLogoName(platformName)));
	...
	private View makeImageItemView(final ImageInfo imageInfo) {
		final View view = LayoutInflater.from(activity).inflate(
				getLayoutRes(activity, "skyblue_editpage_inc_image_layout"), null);
