...
public class SecureKeyBoard extends PopupWindow implements BaseKey.KeyListener {
     ...
     */
    public SecureKeyBoard(Activity context, KeyboardParams keyboardParams, EditText editText) {
        this(context, keyboardParams, editText,null);
    }
     ...
     */
    public SecureKeyBoard(Activity context, KeyboardParams keyboardParams, EditText editText, KeyBoardListener callBack) {
        super(context);
        mContext = context;
        ...
        disableShowSoftInput(editText); //禁止系统键盘弹出
    ...
    public void show() {
        if (!isShowing()) {
            showAtLocation(editText, Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL, 0, 0);
        }
    }
    ...
    public void showAtLocation(View parent, int gravity, int x, int y, View aboveView) {
        hideSystemKeyboard((Activity) mContext);
...
//        }
     ...
     */
    private void disableShowSoftInput(TextView editText) {
