...
import org.xsl781.http.HttpParams;
import org.xsl781.http.core.CachedTask;
import org.xsl781.http.download.I_FileLoader;
import org.xsl781.http.download.SimpleDownloader;
...
public class Http {
    ...
    public void download(String url, File save, HttpCallBack callback) {
        download(url, save, new SimpleDownloader(httpConfig, callback), callback);
    }
     ...
     * @param callback
     */
    public void download(String url, File save, I_FileLoader downloader, HttpCallBack callback) throws RuntimeException{
        try {
            if (save.exists()) {
                save.delete();
            }
    ...
    private class VolleyTask extends CachedTask<String, Object, String> {
        ...
        public VolleyTask(Method requestMethod, String uri, HttpParams params, HttpCallBack callback) {
        ...
        protected String doConnectNetwork(String... uris) throws Exception { // 参数uris留待以后使用
            ...
            try {
                HttpURLConnection conn = openConnection(requestMethod, uri, params);
     ...
     * @throws IOException
     */
    private HttpURLConnection openConnection(Method requestMethod, String uri, HttpParams params)
            throws IOException {
        URL url = new URL(uri);
        ...
        switch (requestMethod) {
            ...
            case POST:
                ...
                httpConfig.httpHeader.put("Content-Type", "application/x-www-form-urlencoded");
                break;
            case FILE:
                connection.setRequestMethod("POST");
                connection.setDoOutput(true);
        ...
        for (String headerName : httpConfig.httpHeader.keySet()) {
            connection.addRequestProperty(headerName, httpConfig.httpHeader.get(headerName));
        }
