...
public class ViewInject {
	...
	public static void toastShowCenter(Context ctx, CharSequence text, int duration, boolean isOk) {
		...
		int Img = 0;
		if (isOk) {
			Img = R.drawable.xsl781_img_toast_center_ok;
		} else {
			Img = R.drawable.xsl781_img_toast_center_error;

		}
		imageCodeProject.setPadding(0, 0, 0, PixelUtil.dip2px(ctx, 8));
		imageCodeProject.setImageResource(Img);
		toastView.addView(imageCodeProject, 0);
		toastView.setBackgroundResource(R.drawable.xsl781_img_toast_center_bg);
		toast.show();
	}
