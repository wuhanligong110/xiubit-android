...
import org.xsl781.db.annotation.Mapping;
...
import org.xsl781.db.model.SQLiteColumn;
import org.xsl781.db.model.SQLiteTable;
...
public final class TableManager {
    ...
    private boolean checkExistAndColumns(SQLiteDatabase db, EntityTable entityTable) {
        if (!Checker.isEmpty(mSqlTableList)) {
            for (SQLiteTable sqlTable : mSqlTableList) {
                if (entityTable.name.equals(sqlTable.name)) {
                    if (Log.isDBPrint) Log.getLog(TableManager.class).d("Table [" + entityTable.name + "] Exist");
                    if (!sqlTable.isTableChecked) {
                        // 表仅进行一次检查，检验是否有新字段加入。
                        sqlTable.isTableChecked = true;
                        if (Log.isDBPrint) Log.getLog(TableManager.class).d("Table [" + entityTable.name + "] check column now.");
                        ...
                        if (!Checker.isEmpty(newColumns)) {
                            ...
                            if (Log.isDBPrint) Log.getLog(TableManager.class).i("Table [" + entityTable.name + "] add " + sum + " new column");
                        }
                    }
                    return true;
                }
        ...
        if (Log.isDBPrint) Log.getLog(TableManager.class).d("Table [" + entityTable.name + "] Not Exist");
        return false;
    }
    ...
    private void putSqlTableIntoList(EntityTable table) {
        if (Log.isDBPrint) Log.getLog(TableManager.class).i("Table [" + table.name + "] Create Success");
    ...
    public ArrayList<SQLiteTable> getAllTablesFromSQLite(SQLiteDatabase db) {
        ...
        if (Log.isDBPrint) Log.getLog(TableManager.class).i("Initialize SQL table start--------------------->");
        Querier.doQuery(db, st, new Querier.CursorParser() {
            ...
            public void parseEachCursor(SQLiteDatabase db, Cursor c) throws Exception {
                ...
                if (Log.isDBPrint) Log.getLog(TableManager.class).d("Find One SQL Table: " + sqlTable);
                list.add(sqlTable);
            }
        });
        if (Log.isDBPrint) Log.getLog(TableManager.class).i("Initialize SQL table end  ---------------------> " + list.size());
        return list;
    }
    ...
    public ArrayList<String> transformSqlToColumns(String sql) {
        if (sql != null) {
            ...
            if (start > 0 && end > 0) {
                ...
                if (Log.isDBPrint) Log.getLog(TableManager.class).w("降级：语义分析表结构（" + colList.toString() + " , Origin SQL is: " + sql);
                return colList;
            }
        }
        return null;
    ...
    public static synchronized EntityTable getTable(Class<?> claxx, boolean needPK) {
        ...
        //if(Log.isDBPrint)Log.i(TAG, "table : " + table + "  , claxx: " + claxx);
        if (table == null) {
            table = new EntityTable();
            ...
            for (Field f : fields) {
                ...
                //if(Log.isDBPrint)Log.i(TAG, "Column : " + Column.class+ "  field: "+ f);
                ...
                // 主键判断
                //if(Log.isDBPrint)Log.i(TAG, "PrimaryKey : " + PrimaryKey.class + "  field: "+ f + " asst:" + AssignType.AUTO_INCREMENT);
                PrimaryKey key = f.getAnnotation(PrimaryKey.class);
                if (key != null) {
                    // 主键不加入属性Map
                    table.key = new org.xsl781.db.model.PrimaryKey(p, key.value());
                    // 主键为系统分配,对类型有要求
                    ...
                    //ORM handle
                    //if(Log.isDBPrint)Log.i(TAG, "Mapping : " + Mapping.class+ " field: "+ f);
