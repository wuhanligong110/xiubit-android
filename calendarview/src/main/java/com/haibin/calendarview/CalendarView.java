...
public class CalendarView extends FrameLayout {
    ...
    /***/
    private int mNowYear, mNowMonth, mNowDay; //今天的年月日
    public Calendar mNowCalendar;
     ...
     * 最小年份和最大年份
     */
    static int mMinYear, mMaxYear, mMinMonth, mMaxMonth;
    ...
    public int mCurrentYear; //当前显示的年份
    public int mCurrentMonth; //当前显示的月份
    public Calendar mCurrentPageCalendar; //当前显示的月份
    private ArrayList<Calendar> monthList;
    private ArrayList<String> monthStrList;
    ...
    public CalendarView(@NonNull Context context, @Nullable AttributeSet attrs) {
        ...
        mMinYear = array.getInt(R.styleable.CalendarView_min_year, -1);
        mMaxYear = array.getInt(R.styleable.CalendarView_max_year, -1);
        mMinMonth = array.getInt(R.styleable.CalendarView_min_month, 1);
        mMaxMonth = array.getInt(R.styleable.CalendarView_max_month, 12);
        ...
        if (mMaxYear == -1 || mMinYear == -1) return;
        setCalendarRange(mMinYear, mMinMonth, mMaxYear, mMaxMonth);
    }

    public void setCalendarRange(int minYear, int minMonth, int maxYear, int maxMonth) {
        mMinYear = minYear;
        mMinMonth = minMonth;
        mMaxYear = maxYear;
        mMaxMonth = maxMonth;
        ...
        if (mMinMonth <= 1) mMinMonth = 1;
        if (mMaxMonth >= 12) mMaxMonth = 12;

        monthList = new ArrayList<>();
        monthStrList = new ArrayList<>();
        int year;
        int month;
        for (year = mMinYear; year <= mMaxYear; year++) {
            if (year == mMinYear) {
                for (month = mMinMonth; month <= 12; month++) {
                    monthList.add(new Calendar(year, month, 1));
                    monthStrList.add(""+month);
                }
            } else if (year == mMaxYear) {
                for (month = 1; month <= mMaxMonth; month++) {
                    monthList.add(new Calendar(year, month, 1));
                    monthStrList.add(""+month);
                }
            } else {
    ...
    private void init(Context context) {
        ...
        mViewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                ...
                calendar.setYear((mMinMonth + position - 1) / 12 + mMinYear);
                calendar.setMonth((mMinMonth + position - 1) % 12 + 1);
                calendar.setDay(1);
        ...
        mInnerListener = new OnInnerDateSelectedListener() {
            ...
            public void onDateSelected(Calendar calendar) {
                //非本月，跳转到对应页面
                if (calendar.getMonth() != mCurrentMonth) {
                    scrollToMonth(calendar.getYear(), calendar.getMonth());
                    return;
                }
        ...
        mNowYear = mSelectedCalendar.getYear();
        if (mMinYear >= mNowYear) mMinYear = mNowYear;
        if (mMaxYear < mNowYear) mMaxYear = mNowYear;
        mSelectLayout.setYearSpan(mMinYear, mMaxYear);
        mNowMonth = mSelectedCalendar.getMonth();
        mNowDay = mSelectedCalendar.getDay();
        mNowCalendar = mSelectedCalendar;
        int y = mSelectedCalendar.getYear() - mMinYear;
        position = 12 * y + mSelectedCalendar.getMonth() - mMinMonth;
        mCurrentYear = mSelectedCalendar.getYear();
        mCurrentMonth = mSelectedCalendar.getMonth();
        ...
        mSelectLayout.setOnMonthSelectedListener(new MonthRecyclerView.OnMonthSelectedListener() {
            ...
            public void onMonthSelected(int year, int month) {
                int position = 12 * (year - mMinYear) + month - mMinMonth;
    ...
    public void scrollToCurrent() {
        mViewPager.setCurrentItem(12 * (mNowYear - mMinYear) + mNowMonth - 1);
    }
    ...
    public void scrollToYear(int year) {
        mViewPager.setCurrentItem(12 * (year - mMinYear) + mNowMonth - 1);
    }
     ...
     * @param listener 监听
     */
    public CalendarView setOnDateChangeListener(OnDateChangeListener listener) {
     ...
     */
    public CalendarView setOnDateSelectedListener(OnDateSelectedListener listener) {
    ...
    private class CalendarViewPagerAdapter extends PagerAdapter {
        ...
        @Override
        public int getCount() {
            return 12 * (mMaxYear - mMinYear) + (mMaxMonth - mMinMonth + 1);
        }
        ...
        public Object instantiateItem(ViewGroup container, int position) {
            ...
            int year = (mMinMonth + position - 1) / 12 + mMinYear;
            int month = (mMinMonth + position - 1) % 12 + 1;
